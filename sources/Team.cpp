...
void ariel::Team::add(Character* new_member){
    ...
    if(new_member == NULL){
        throw std::runtime_error("new member is null");
        return;
    }
    if(new_member->isAlive() == false){
        throw std::runtime_error("new member is dead");
        return;
    }
              ...
              if(m_team_order[k]->getType() == "Ninja"){
               ...
               if(victim->isAlive() == false){
                   other->num_alive--;
                    ...
                    victim = choose_victim(other->m_team,this->m_leader,other->num_of_worriers);
                    k--;
               }
               if(other->stillAlive() == 0){
